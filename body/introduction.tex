...
\begin{enumerate}[leftmargin=80pt]
    ...
    Web application accepts a \textbf{monthly Assembly Master Plan} in Excel format 
    and uses it as input for plastic injection molding production planning. 
    The input consists of the following elements:

    \begin{itemize}
  \item \textbf{Planning Period}
  \begin{itemize}
    \item Number of working days in the month used as the scheduling horizon.
  \end{itemize}

  \item \textbf{Injection Molding Machines}
  \begin{itemize}
    \item Machine ID, name, group, tonnage capacity, available working hours per day, and efficiency.
  \end{itemize}

  \item \textbf{Molds}
  \begin{itemize}
    \item Mold ID, mold group, and required tonnage for machine compatibility checking.
  \end{itemize}

  \item \textbf{Components / Parts}
  \begin{itemize}
    \item Component ID and name
    \item Required quantity and cycle time
    \item Assigned mold and color
    \item Due date and lead time
    \item Prerequisite components for assembly relationships
  \end{itemize}

  \end{itemize}


This input enables the system to automatically decompose the assembly plan into required plastic parts and generate a feasible production plan under real factory constraints. Users are also able to manually modify the input data through the web interface.
